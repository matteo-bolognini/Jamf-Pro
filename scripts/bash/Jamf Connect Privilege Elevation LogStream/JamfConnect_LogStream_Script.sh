...
log show --debug --info --style compact --predicate '(subsystem == "com.jamf.connect.daemon") && (category == "PrivilegeElevation")' --last 60m >> "$path/Connect_Daemon_Elevation_Logs.log"

# Collect Jamf Connect Menubar Elevation Logs
echo "Collecting Jamf Connect Menubar Elevation Logs..."
log show --debug --info --style compact --predicate '(subsystem == "com.jamf.connect") && (category == "PrivilegeElevation")' --last 60m >> "$path/Connect_Menubar_Elevation_Logs.log"
